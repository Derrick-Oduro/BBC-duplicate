        ...
        <div class="card-body p-4">
            ...
            @if(isset($posts) && count($posts) > 0)
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 ">
                        ...
                        <div class="bg-white rounded-lg p-6">
                            <div class="flex gap-5 flex-row-reverse">
                                @if($featuredPost->image)
                                    <div class="flex">
                                        <div class=" lg:w-96 lg:h-72 flex-shrink-0">
                                ...
                                <div class="flex-left min-w-0">
                                    <h1 class="text-xl lg:text-2xl font-bold mb-3 leading-tight text-gray-900">
                                        <a href="/posts/{{ $featuredPost->id }}" class="hover:underline transition-colors">
                                            {{ $featuredPost->title ?? 'No Title' }}
                                        </a>
                                    </h1>
                                    <p class="text-gray-600 mb-4 text-sm lg:text-base line-clamp-4 leading-relaxed">{{ Str::limit(strip_tags($featuredPost->body ?? 'No content available'), 200) }}</p>
                    ...
                    <div class="space-y-5">
                        @foreach($posts->take(-4)->reverse()->skip(1) as $index => $post)
                            <div class="bg-white border-b border-gray-200 p-4 hover:bg-gray-50 transition-colors {{ $loop->first ? 'pb-6' : '' }}">
                                @if($post->image && $loop->first)
                                ...
                                <div>
                                    <h3 class="text-sm font-bold mb-1 leading-tight text-gray-900 line-clamp-2">
                                        <a href="/posts/{{ $post->id }}" class="hover:underline transition-colors">
                                            {{ $post->title ?? 'No Title' }}
                                        </a>
                                    </h3>
                                    <p class="text-gray-600 text-xs mb-1 line-clamp-4 leading-normal">{{ Str::limit(strip_tags($post->body ?? 'No content available'), 100) }}</p>
                            ...
                            <div class="bg-white border-gray-200 p-3 hover:bg-gray-50 transition-colors flex-shrink-0 w-40 h-45">
                                ...
                                <div>
                                    <h3 class="text-sm font-bold mb-2 leading-tight text-gray-900 line-clamp-2">
                                        <a href="/posts/{{ $post->id }}" class="hover:underline line transition-colors">
                                            {{ $post->title ?? 'No Title' }}
                                        </a>
                                    </h3>
